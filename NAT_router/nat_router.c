...
{
    uint32_t acc = 0; uint16_t *d = (uint16_t *)vdata;
    for (; len > 1; len -= 2) acc += *d++;
    if (len) acc += *(uint8_t *)d;
...
static uint16_t l4_checksum(struct ip *iph, void *l4, size_t len)
{
    ...
    pseudo.src = iph->ip_src.s_addr; pseudo.dst = iph->ip_dst.s_addr;
    pseudo.zero = 0; 
    pseudo.proto = iph->ip_p; 
    pseudo.l4len = htons(len);
    
    uint32_t acc = 0; uint16_t *p=(uint16_t *)&pseudo;
    for (size_t i=0;i<sizeof(pseudo)/2;++i) acc+=*p++;
    p = (uint16_t *)l4; for (; len>1; len-=2) acc+=*p++;
    if (len) acc+=*(uint8_t*)p; while (acc>>16) acc=(acc&0xFFFF)+(acc>>16);
...
{
    int fd=socket(AF_PACKET,SOCK_RAW,htons(ETH_P_ALL)); if(fd==-1){perror("socket");exit(1);} struct sockaddr_ll s={0};
    s.sll_family=AF_PACKET; s.sll_ifindex=if_nametoindex(ifname); s.sll_protocol=htons(ETH_P_ALL);
    if(bind(fd,(struct sockaddr*)&s,sizeof(s))==-1){perror("bind");exit(1);} return fd;
}
static uint32_t iface_ip(const char *ifname)
{
    int s=socket(AF_INET,SOCK_DGRAM,0); struct ifreq ifr={0}; strncpy(ifr.ifr_name,ifname,IFNAMSIZ-1);
    if(ioctl(s,SIOCGIFADDR,&ifr)==-1){perror("SIOCGIFADDR");exit(1);} close(s);
    return ((struct sockaddr_in*)&ifr.ifr_addr)->sin_addr.s_addr;
}

/* ---------------- payload dump ---------------- */
static void dump_payload(const unsigned char *d,int len)
{int n=len>256?256:len;printf("    Payload (%d bytes,showing %d):\n    ",len,n);
    for(int i=0;i<n;++i)printf("%02X ",d[i]);printf("\n    ");
    for(int i=0;i<n;++i)putchar((d[i]>=32&&d[i]<=126)?d[i]:'.');
    putchar('\n');}

/* ---------------- proto name helper ---------------- */
static const char *proto_name(uint8_t p){switch(p){case IPPROTO_TCP:return"TCP";case IPPROTO_UDP:return"UDP";case IPPROTO_ICMP:return"ICMP";default:return"OTH";}}

/* ---------------- cleanup ---------------- */
static void cleanup(int sig){if(raw_int!=-1)close(raw_int);if(raw_ext!=-1)close(raw_ext);puts("\n[+] NAT stopped.");exit(0);} 
...
{
    if(argc!=3){fprintf(stderr,"Usage: %s <int_if> <ext_if>\n",argv[0]);return 1;}
    srand(time(NULL)); const char *int_if=argv[1],*ext_if=argv[2];
    raw_int=create_raw(int_if); raw_ext=create_raw(ext_if);
    signal(SIGINT,cleanup); signal(SIGTERM,cleanup);
    unsigned char buf[BUF_SZ]; puts("[+] User‑space NAT running… Ctrl‑C to quit.\n");
    int_if_ip=iface_ip(int_if);ext_if_ip=iface_ip(ext_if);
    ...
    while(1){
        nat_gc(); fd_set rd; FD_ZERO(&rd); FD_SET(raw_int,&rd); FD_SET(raw_ext,&rd); int maxfd=(raw_int>raw_ext?raw_int:raw_ext);
        ...
        if(FD_ISSET(raw_int,&rd)){
            ssize_t n=recv(raw_int,buf,BUF_SZ,0); if(n<=0){continue;} 
            ...
            void *l4=(unsigned char*)ip+ip->ip_hl*4; uint16_t id_or_port=0; size_t hdr_add=0;
            ...
            {   struct udphdr *u=l4; id_or_port=ntohs(u->source);
                 hdr_add=sizeof(struct udphdr); u->check=0;
            }
            ...
            if(ip->ip_p==IPPROTO_TCP){((struct tcphdr*)l4)->source=htons(e->ext_port);} 
            else if(ip->ip_p==IPPROTO_UDP){((struct udphdr*)l4)->source=htons(e->ext_port);} /* ICMP keep id */
            ip->ip_sum=0; ip->ip_sum=checksum(ip,ip->ip_hl*4);
            ...
            int hdr_len=sizeof(*eth)+ip->ip_hl*4+hdr_add; int pay_len=n-hdr_len; 
            //if(pay_len>0)dump_payload(buf+hdr_len,pay_len);            
            if(send_out_via_s1(raw_ext, (uint8_t*)ip, n-sizeof(struct ether_header), (char*)gw_mac, ext_if)==-1) perror("send ext");

        }

        /* -------- external → internal -------- */
        if(FD_ISSET(raw_ext,&rd)){
            ssize_t n=recv(raw_ext,buf,BUF_SZ,0); if(n<=0){continue;}
            struct ether_header *eth=(void*)buf;
            
            if(ntohs(eth->ether_type)!=ETHERTYPE_IP){continue;}
            struct ip *ip=(void*)(buf+sizeof(*eth));
            if(ip->ip_src.s_addr==ext_if_ip){continue;}

            void *l4=(unsigned char*)ip+ip->ip_hl*4; uint16_t id_or_port=0; size_t hdr_add=0;
            if(ip->ip_p==IPPROTO_TCP){struct tcphdr *t=l4; id_or_port=ntohs(t->dest); hdr_add=t->doff*4; t->check=0;}
            else if(ip->ip_p==IPPROTO_UDP)
            {struct udphdr *u=l4; id_or_port=ntohs(u->dest); hdr_add=sizeof(struct udphdr); u->check=0;}
            else if(ip->ip_p==IPPROTO_ICMP)
            {struct icmphdr *c=l4; id_or_port=ntohs(c->un.echo.id); hdr_add=sizeof(struct icmphdr); c->checksum=0;}
            else continue;
            ...
            if(ip->ip_p==IPPROTO_TCP){((struct tcphdr*)l4)->dest=htons(e->int_port);} 
            else if(ip->ip_p==IPPROTO_UDP){((struct udphdr*)l4)->dest=htons(e->int_port);} /* ICMP keep id */

            ip->ip_sum=0; ip->ip_sum=checksum(ip,ip->ip_hl*4);
            size_t l4len=ntohs(ip->ip_len)-ip->ip_hl*4;
            if(ip->ip_p==IPPROTO_TCP||ip->ip_p==IPPROTO_UDP)
            {   uint16_t cks=l4_checksum(ip,l4,l4len); 
                if(ip->ip_p==IPPROTO_TCP){((struct tcphdr*)l4)->check=cks;} 
                else ((struct udphdr*)l4)->check=cks;
            }
            else if(ip->ip_p==IPPROTO_ICMP){((struct icmphdr*)l4)->checksum=checksum(l4,l4len);} 
...
static int get_iface_mac(const char *ifname, uint8_t mac[6])
{
    ...
    if (ioctl(s, SIOCGIFHWADDR, &ifr) == -1) { perror("SIOCGIFHWADDR"); close(s); return -1; }
    close(s);
...
static int get_default_gw(const char *iface_out, size_t iflen, uint32_t *gw_ip)
{
    ...
    if (!f) { perror("open route"); return -1; }
...
{
    uint8_t src_mac[6];
    if (get_iface_mac(iface, src_mac)   == -1) return -1;
